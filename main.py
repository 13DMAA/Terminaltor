...
bot = Binance(
    API_KEY='SVIuPqVJhe1T5G7wwEGx8Ry71GD0s2S04jDMZ5CCImCQZUazVuwQp93N7MkTxyC3',
    API_SECRET='4uMjBZLmQGxeUCWy9eTY0on44c4Uv5hKhAV5KvNbfHltZQkLVv5BOh1t89U5vORh'
)
...
class Binance_page(Screen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        last_y = 0
        self.count_orders = 100
        for i in range(self.count_orders//2):
            self.ids.Order_book_binance.add_widget(Label(text=f'', 
            pos = (0, i*self.ids.Order_book_binance.shift-
            (self.ids.Order_book_binance.height/2-self.ids.Order_book_binance.shift)), color='orangered'))
            last_y = i*self.ids.Order_book_binance.shift-(self.ids.Order_book_binance.height/2-self.ids.Order_book_binance.shift)
        for i in range(self.count_orders//2):
            self.ids.Order_book_binance.add_widget(Label(text=f'', 
            pos = (0, i*self.ids.Order_book_binance.shift-
            (self.ids.Order_book_binance.height/2-self.ids.Order_book_binance.shift)-last_y), color='lime'))

    def get_depth(self):
        while True:
            try:
                depth = bot.depth(symbol='BTCUSDT', limit = self.count_orders//2)
                for i, el in enumerate(self.ids.Order_book_binance.children):
                    if i>=self.count_orders//2:
                        el.text=f'{float(depth["asks"][i-self.count_orders//2][0]):.2f} {float(depth["asks"][i-self.count_orders//2][1]):.2f}'
                    else:
                        el.text=f'{float(depth["bids"][self.count_orders//2-i-1][0]):.2f} {float(depth["bids"][self.count_orders//2-i-1][1]):.2f}'
            except Exception as err:
                print(f'Error, {err}')

class Terminaltor_v1_0App(App):
    def build(self):
        sm = ScreenManager()
        b_p_obj = Binance_page(name='binance_page')
        threading.Thread(target=b_p_obj.get_depth, daemon=True).start()
        sm.add_widget(b_p_obj)
        return sm

if __name__ == '__main__':
    Terminaltor_v1_0App().run()
